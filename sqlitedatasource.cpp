...
void SQLiteDataSource::createTable()
{
    ...
    std::string sqlStr = "CREATE TABLE IF NOT EXISTS contactbook"
...
void SQLiteDataSource::fillContactFromRow(sqlite3_stmt *s, Contact& c)
{
    ...
    c.pincode = reinterpret_cast<const char*>(sqlite3_column_text(s, 6));
}
...
ErrorInfo SQLiteDataSource::getContact(Contact::ContactId id, Contact& c)
{
    ...
    std::string sqlStr = "SELECT * FROM contactbook WHERE id=?;";
...
{
    //create sql prepared statement
    std::string sqlStr = "SELECT * FROM contactbook;";
...
{
    //create sql prepared statement
    std::string sqlStr = "INSERT INTO contactbook VALUES("
                        "NULL,?,?,?,?,?,?);";
    ...
    sqlite3_bind_text(insertStatement.get(), 6, c.pincode.c_str(), -1, SQLITE_STATIC);
...
{
    //create sql prepared statement
    std::string sqlStr = "UPDATE contactbook SET "
                         "firstname=?, lastname=?,"
                         "phonenum=?, address=?,"
                         "email=?, pincode=?  WHERE id=?;";
    ...
    sqlite3_bind_text(updateStatement.get(), 6, c.pincode.c_str(), -1, SQLITE_STATIC);
...
ErrorInfo SQLiteDataSource::deleteContact(Contact::ContactId id)
{
    ...
    //create sql prepared statement
    std::string sqlStr = "DELETE FROM contactbook WHERE id=?;";
...
{
    //create sql prepared statement
    std::string sqlStr = "DELETE FROM contactbook;";
